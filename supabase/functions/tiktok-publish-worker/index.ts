...
Deno.serve(async (req: Request) => {
  ...
  let query = db
    .from("publish_queue")
    .select("id,idempotency_key,publication_id,provider,status,payload,provider_publish_id,attempt_count,scheduled_for")
    .eq("provider", "tiktok");

  if (queueId) query = query.eq("id", queueId);
  else query = query.in("status", ["queued", "publishing"]).or(`status.eq.publishing,scheduled_for.is.null,scheduled_for.lte.${nowIso}`).order("scheduled_for", { ascending: true, nullsFirst: false }).order("created_at", { ascending: true }).limit(1);
  ...
  if (!accessToken) return json({ ok: false, state: "credentials_missing", queue_id: row.id, required_secret: "TIKTOK_ACCESS_TOKEN" }, 412);
  ...
  if (row.provider_publish_id) {
    ...
    if (terminalSuccess && row.publication_id) {
    ...
    return json({ ok: statusRes.ok, state: "polled", queue_id: row.id, publication_id: row.publication_id, provider_status: providerStatus, publication_sync_warning: publicationSyncWarning });
  }
  ...
  if (!apiOk) {
    await db.from("publish_queue").update({
      status: "failed", failed_at: new Date().toISOString(), provider_status: "INIT_FAILED", provider_result: initBody,
      error_code: initBody?.error?.code ?? `http_${initRes.status}`,
      error_message: initBody?.error?.message ?? "TikTok init request failed", updated_at: new Date().toISOString(),
    }).eq("id", row.id);
  ...
  return json({ ok: true, state: "submitted", queue_id: row.id, publication_id: row.publication_id, publish_id: publishId });
});