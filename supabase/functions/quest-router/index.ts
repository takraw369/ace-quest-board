...
function focusAxes(meta: Record<string, any>) {
  const ace = meta?.current_ace ?? null;
  const flow = meta?.current_flow ?? null;
  const axis = String(ace?.result_axis ?? "").toUpperCase();
  const axisMap: Record<string, string[]> = {
    BODY: ["body"],
    COGNITION: ["attention", "meta"],
    EMOTION: ["emotion", "impulse", "meta"],
    ACTION: ["action_planning", "impulse"],
  };
  if (axisMap[axis]) return axisMap[axis];

  const bottleneck = String(flow?.bottleneck ?? "").toLowerCase();
  const flowMap: Record<string, string[]> = {
    body: ["body"],
    mind: ["attention", "meta", "emotion"],
    action: ["action_planning", "impulse"],
    environment: ["attention", "action_planning"],
  };
  return flowMap[bottleneck] ?? [];
}

function scoreCandidate(item: CatalogItem, params: {
  timeBudget: number;
  ...
  if (overBy <= 0) {
    score += 60;
    reasons.push(`${item.estimated_minutes}分で今の時間に収まる`);
    score += Math.max(0, 20 - Math.abs(params.timeBudget - item.estimated_minutes));
  } else {
    score -= overBy * 8;
  }

  const overlap = item.observation_axes.filter((axis) => params.axes.includes(axis));
  if (overlap.length > 0) {
    score += Math.min(60, overlap.length * 25);
    reasons.push(`今のACE/FLOWと${overlap.length}軸がつながる`);
  }

  if (params.attention === "light") {
    if (item.estimated_minutes <= 7) score += 25;
    if (item.difficulty <= 2) score += 15;
  } else {
    if (item.estimated_minutes >= 10) score += 20;
    if (item.difficulty >= 2) score += 10;
  }

  // When signals are otherwise equal, progress through the curriculum rather than jumping ahead.
...
function candidatePayload(candidate: RankedCandidate) {
  ...
  return {
    ...
    observation_axes: item.observation_axes,
    tags: item.tags,
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const ageBand = String(body?.age_band ?? "").trim();
    ...
    if (!ageBand) {
      return Response.json({ ok: false, error: "age_band_required" }, { status: 400, headers: cors });
    }
    ...
    const available = (catalog ?? []).filter((item: CatalogItem) => !completedRefs.has(item.recommendation_ref));
    const axes = focusAxes(contact.metadata ?? {});
    let ranked = available
      .map((item: CatalogItem) => ({ item, ...scoreCandidate(item, { timeBudget, attention, axes }) }))
      .sort((a: RankedCandidate, b: RankedCandidate) => b.score - a.score || a.item.global_order - b.item.global_order);

    // Respect the user's time first. If that would leave no options, fall back to the nearest available Quest.
    ...
    if (ranked.length === 0) {
      return Response.json({
        ...
        engine: ENGINE,
        candidates: [],
        reason: "no_available_quest_for_context",
        context: { age_band: ageBand, capability, time_budget_minutes: timeBudget, attention_level: attention, focus_axes: axes },
      }, { headers: cors });
    }

    if (action === "select") {
      const selectedRef = String(body?.recommendation_ref ?? "");
      const selected = ranked.find((candidate) => candidate.item.recommendation_ref === selectedRef);
      if (!selected) {
        return Response.json({ ok: false, error: "selected_quest_not_in_current_candidates" }, { status: 400, headers: cors });
      }

      const { data: currentRec, error: currentRecError } = await supabase
        .from("education_recommendations")
        .select("id,metadata,alternative,status")
        .eq("person_id", personId)
        .eq("recommendation_type", "quest")
      ...
      if (!currentRec) {
        return Response.json({ ok: false, error: "base_quest_recommendation_missing", next: "refresh_recommendations" }, { status: 409, headers: cors });
      }

      const item = selected.item;
      const relatedNodeIds = Array.isArray(item.metadata?.related_node_ids) ? item.metadata.related_node_ids : [];
      const nodeId = relatedNodeIds.length > 0 ? String(relatedNodeIds[0]) : null;
      const reasonParts = selected.reasons.length > 0 ? selected.reasons.join("・") : "今の条件に合う";
      const reason = `${reasonParts}ため、このQuestを候補にしました。正解ではなく、今の自分を知るための小さな実験です。`;
      const alternative = {
        ...(currentRec.alternative ?? {}),
        duration: `${item.estimated_minutes}分Quest`,
        action: item.instruction,
        experiment: item.experience,
        quest_catalog: {
          quest_key: item.quest_key,
          capability: item.capability,
          age_band: item.age_band,
      ...
      const metadata = {
        ...
        router_context: { time_budget_minutes: timeBudget, attention_level: attention, focus_axes: axes },
      };

      const { data: updated, error: updateError } = await supabase
        .from("education_recommendations")
        .update({ recommendation_ref: item.recommendation_ref, reason, alternative, metadata })
        .eq("id", currentRec.id)
        .select("id,recommendation_type,recommendation_ref,destination,reason,confidence,alternative,status,generated_at,metadata")
        .single();
      if (updateError) throw updateError;

      await supabase.from("funnel_events").insert({
        contact_id: personId,
        event_type: "quest_router_selected",
        channel: "pwa",
        payload: {
          engine: ENGINE,
          recommendation_ref: item.recommendation_ref,
      ...
      return Response.json({ ok: true, engine: ENGINE, selected: candidatePayload(selected), recommendation: updated }, { headers: cors });
    }
    ...
    return Response.json({
      ...
      engine: ENGINE,
      context: { age_band: ageBand, capability, time_budget_minutes: timeBudget, attention_level: attention, focus_axes: axes },
      candidates: ranked.map(candidatePayload),
    }, { headers: cors });
  } catch (error) {
