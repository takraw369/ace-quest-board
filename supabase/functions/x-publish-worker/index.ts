...
async function resolveHarnessAccount(base: string, key: string) {
  ...
  if (configured) return { ok: true, xAccountId: configured, source: "runtime_config" };
...
async function publishViaHarness(text: string, mediaIds: string[] | undefined) {
  ...
  if (!account.ok || !account.xAccountId) {
    return { ok: false, id: null, body: account.body ?? { error: account.error }, status: account.status ?? 502, adapter: "x_harness", stage: "account_lookup" };
  }

  const createBody: Record<string, unknown> = { xAccountId: account.xAccountId, text };
  ...
  return { ok: res.ok && !!id, id, body, status: res.status, adapter: "x_harness", x_account_id: account.xAccountId };
}
...
Deno.serve(async (req: Request) => {
  ...
  let q = db
    .from("publish_queue")
    .select("id,idempotency_key,publication_id,source_ref,provider,status,payload,provider_publish_id,attempt_count,scheduled_for")
    .in("provider", ["x", "twitter"]);
  if (queueId) q = q.eq("id", queueId);
  else q = q.eq("status", "queued").or(`scheduled_for.is.null,scheduled_for.lte.${nowIso}`).order("scheduled_for", { ascending: true, nullsFirst: false }).order("created_at", { ascending: true }).limit(1);
  ...
  if (row.scheduled_for && new Date(row.scheduled_for).getTime() > Date.now()) return json({ ok: true, state: "scheduled", queue_id: row.id, scheduled_for: row.scheduled_for });
  ...
  const { data: claimed, error: claimError } = await db.from("publish_queue").update({
    status: "publishing", publishing_at: claimedAt, attempt_count: row.attempt_count + 1, updated_at: claimedAt,
  ...
  if (!result || !result.ok || !result.id) {
    ...
    await db.from("publish_queue").update({
      status: "failed", failed_at: now, provider_status: `HTTP_${result?.status ?? 0}`,
      provider_result: { adapter: result?.adapter ?? "none", response: result?.body ?? null },
      error_code: result?.body?.title ?? result?.body?.errors?.[0]?.type ?? result?.body?.error ?? "x_publish_failed",
      error_message: result?.body?.detail ?? result?.body?.error ?? "X publish failed", updated_at: now,
    }).eq("id", row.id);
  ...
  const { error: updateError } = await db.from("publish_queue").update({
    status: "published", provider_publish_id: result.id, provider_status: "PUBLISHED",
    provider_result: { adapter: result.adapter, response: result.body }, published_at: now,
    next_poll_at: null, error_code: null, error_message: null, updated_at: now,
  }).eq("id", row.id);
  if (updateError) return json({ error: "queue_update_failed", detail: updateError.message, post_id: result.id }, 500);

  let publicationSyncWarning: string | null = null;
  if (row.publication_id) {
    const { error: publicationError } = await db.from("content_publications").update({
      status: "published", published_at: now, updated_at: now,
    }).eq("id", row.publication_id).in("status", ["draft", "ready", "scheduled"]);
    if (publicationError) {
      publicationSyncWarning = publicationError.message;
      console.error("content_publication_sync_failed", publicationError.message);
    }
  }
  ...
  return json({
    ...
    publication_sync_warning: publicationSyncWarning,
  });
});