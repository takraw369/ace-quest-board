      ...
      <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-3 px-4 py-2.5 md:px-7">
        <Link href="/" className="flex min-w-0 items-center gap-2.5">
          ...
          <span className="min-w-0">
            <span className="block truncate text-[10px] font-bold tracking-[.18em] text-[#f2f5f9]">FLOW OS</span>
            <span className="hidden text-[8px] tracking-[.12em] text-[#718097] sm:block">KNOWLEDGE → WANT TO → QUEST</span>
          </span>
        </Link>
          ...
          {LAYERS.map((layer) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center justify-center gap-1.5 rounded-full px-2.5 py-1.5 text-[9px] font-semibold tracking-wide transition sm:px-3.5 sm:text-[10px] ${
                  active
                    ? 'bg-[#ff8a1f] text-[#071426] shadow-[0_5px_20px_rgba(255,138,31,.20)]'
                    : 'text-[#aeb9c8] hover:bg-white/[0.06] hover:text-white'
                }`}
              >
                <span className="font-serif text-xs sm:text-sm">{layer.kanji}</span>
